    ...
    public function depositConfirm(Request $request, $transaction)
    {
        if ($request && $request != null) {
            ...
            if ($user) {
                Auth::login($user);
                $user = Auth()->user();
                $request->session()->put('logged_in_as', 'patient');
            }
        }
        $track = $transaction;
        ...
        if ($request->appointment_id || $request->service_id) {
            $appointment = BookAppointment::where('id', $request->appointment_id)->first();
            $service = BookedService::where('id', $request->service_id)->first();

            if ($appointment && $order->gateway->code == 'bank-transfer') {
                return redirect(route('patients.appointment_bank_transfers', ['appointment_id' => $appointment->id]));
            }
            if ($service && $order->gateway->code == 'bank-transfer') {
                return redirect(route('patients.service_bank_transfers', ['service_id' => $service->id]));
            }
            if ($appointment && $order->gateway->code == 'stripe') {
                // return redirect(route('patients.appointment_stripe_transfers', ['appointment_id' => $appointment->id]));
                return Inertia::location(route('patients.appointment_stripe_transfers', ['appointment_id' => $appointment->id]));
            }
            if ($service && $order->gateway->code == 'stripe') {
                // return redirect(route('patients.service_stripe_transfers', ['service_id' => $service->id]));
