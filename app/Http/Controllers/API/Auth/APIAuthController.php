...
class APIAuthController extends Controller
{
    ...
    public function submitLoginForm(LoginRequest $request)
    {
        ...
        if ($user) {
            ...
            if ($request->login_as == 'doctor' && !$user->hasRole(Role::$Doctor)) {
                $response = generateResponse(null,false,"The provided email and password do not match",null,'collection');
                return response()->json($response);
            }
            if ($request->login_as == 'patient' && !$user->hasRole(Role::$Patient)) {
                $response = generateResponse(null,false,"The provided email and password do not match",null,'collection');
                return response()->json($response);
            }
            if ($request->login_as == 'clinic' && !$user->hasRole(Role::$Clinic)) {
                $response = generateResponse(null,false,"The provided email and password do not match",null,'collection');
                return response()->json($response);
            }
    ...
    public function submitRegisterForm(Request $request)
    {
        ...
        if($request->login_as == 'doctor'){
            ...
            $user->doctor()->create(['pricing_plan_id' => $pricing_plan->id ?? null,'first_name' => $data['first_name'], 'last_name' => $data['last_name'],'zip_code' => $data['zip_code'] ?? null]);
        }
        if($request->login_as == 'patient'){
            $user->patient()->create(['first_name' => $data['first_name'], 'last_name' => $data['last_name'],'zip_code' => $data['zip_code'] ?? null]);
        }
        if($request->login_as == 'clinic'){
            $pricing_plan = getClinicDefaultPricingPlan();
            $user->clinic()->create(['pricing_plan_id' => $pricing_plan->id ?? null,'first_name' => $data['first_name'], 'last_name' => $data['last_name'],'zip_code' => $data['zip_code'] ?? null]);
        }
    ...
    public function socialLogin(SocialLoginRequest $request){
     ...
     public function loginUser($user ,$request, $data){
        ...
        $token = $user->createToken('MyApp',[]);
        $success['token'] =  $token->accessToken;
        $request->session()->put('logged_in_as', $data['login_as'] ?? 'patient');
        $response = generateResponse($success,true,"Successfully Login",null,'collection');
        return $response;
    }
