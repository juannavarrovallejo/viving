...
// Patients
$patient = $api->patient->create(array('name' => 'Razorpay User', 'email' => 'patient@razorpay.com')); // Creates patient
$patient = $api->patient->fetch($patientId); // Returns a particular patient
$patient = $api->patient->edit(array('name' => 'Razorpay User', 'email' => 'patient@razorpay.com')); // Edits patient

// Tokens
$token  = $api->patient->token()->fetch($tokenId); // Returns a particular token
$tokens = $api->patient->token()->all($options); // Returns array of token objects
$api->patient->token()->delete($tokenId); // Deletes a token
...
$link  = $api->invoice->create(arary('type' => 'link', 'amount' => 500, 'description' => 'For XYZ purpose', 'patient' => array('email' => 'test@test.test')));
$link->cancel();
$link->notifyBy('sms');
...
$plans         = $api->plan->all();
$subscription  = $api->subscription->create(array('plan_id' => 'plan_7wAosPWtrkhqZw', 'patient_notify' => 1, 'total_count' => 6, 'start_at' => 1495995837, 'addons' => array(array('item' => array('name' => 'Delivery charges', 'amount' => 30000, 'currency' => 'INR')))));
