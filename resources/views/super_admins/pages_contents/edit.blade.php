...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // CKEDITOR.replace('{{$pages_content->name}}_{{$language->code}}');
            ClassicEditor.create(document.querySelector('#{{$pages_content->name}}_{{$language->code}}')).catch(error => { console.error(error); });
            @endif
            @endforeach
            @endforeach
        });
