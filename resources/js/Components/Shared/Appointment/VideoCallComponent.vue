<template>
  ...
  <div
    ...
    "
  >
    <div class="col-md-12 align-items-center d-flex justify-content-center mt-4 gap-2">
      <button
        type="button"
        @click="joinEvent"
        v-if="
        ...
        class="btn btn-primary width rounded-circle"
      >
      ...
      <button
        ...
        @click="leaveEvent"
        v-if="disableJoin"
        class="btn btn-danger width rounded-circle width ms-0"
        plain
        :disabled="!disableJoin"
      >
        <img src="@/images/icons/call.svg" class="" alt="" width="30" />
      </button>
      ...
      <button
        ...
        "
        class="btn btn-danger width rounded-circle"
        plain
      >
      ...
      <button
        ...
        "
        class="btn btn-secondary opacity-50 width rounded-circle"
        plain
      >
        <img src="@/images/icons/mike.svg" class="" alt="" width="30" />
      </button>
      <button
        type="button"
        ...
        plain
      >
        <i class="bi bi-skip-start-fill fs-1"></i>
      </button>
    </div>
  </div>
  ...
  <div class="col-12" v-if="$page.props.auth.logged_in_as == 'patient'">
    ...
    <div
      class="row justify-content-center rounded-4"
      style="background-color: #e5f0f9"
      v-else-if="!incomingCall && appointment.appointment_status_code == 2 && !autoJoin"
    >
      <div class="col-md-12">
    ...
    <div
      ...
      style="background-color: #e5f0f9"
      v-else-if="!incomingCall && appointment.appointment_status_code == 1 && !autoJoin"
    >
      <div class="col-md-12">
  ...
  <!-- Modal -->
  <Modal :id="'videoCallModal'" :aria-labelledby="'videoCallModalLabel'">
...
export default defineComponent({
  ...
  props: ["appointment", "is_call"],
  methods: {
    modifyCallStatus() {
      this.$emit("update-call", false); // Emit the modified value back to parent
    },
    async joinEvent() {
    ...
    async CameraToggle() {
      try {
        ...
        this.rtc.toggleStreamCamera();

        // stream.getTracks().forEach((track) => track.stop());
        this.audioPermissionMessage = null;

        // Stop all video tracks to disable the camer

        // Set a message or handle the state if needed
        this.cameraPermissionMessage = "Camera stopped successfully";
      } catch (error) {
        console.error("Unable to stop the camera or permission denied", error);
        this.cameraPermissionMessage =
          "Unable to stop the camera or permission denied by system";
        // this.$toast.error(error); // Show error toast if you are using a toast notification library
      }
    },
    async MicToggle() {
      try {
        // Request access to the camera
        this.rtc.toggleStreamMic();
        ...
        // Set a message or handle the state if needed
        this.micPermissionMessage = "Camera stopped successfully";
      } catch (error) {
        console.error("Unable to stop the Mic or permission denied", error);
        this.cameraPermissionMessage =
          "Unable to stop the Mic or permission denied by system";
      }
    },
