<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="doctor_services.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __("no record found") }}
          </td>
        </tr>
        <tr v-for="service in doctor_services.data" :key="service.id">
...
import AddEditServiceModal from "@/Components/Doctors/Services/AddEditServiceModal.vue";
import ViewServiceModal from "@/Components/Doctors/Services/ViewServiceModal.vue";
import DeleteServiceModal from "@/Components/Doctors/Services/DeleteServiceModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getDoctorServices(loading_more);
    },
    clearModalData() {
      this.modal_service = null;
      this.key++;
    },
    getDoctorServices(loading_more) {
      axios
        .post(this.route("doctors.doctor_services.filter"), this.filter)
        .then((res) => {
          const data = res.data.data;
          if (loading_more) {
            this.doctor_services.data = this.doctor_services.data.concat(
              data.data
            );
          } else {
            this.doctor_services.data = data.data;
          }
          this.doctor_services.links = data.links;
          this.doctor_services.meta = data.meta;
          this.fetching = false;
        });
    },
  },
