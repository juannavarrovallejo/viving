...
<template>
  <div class="card rounded-3">
    <div class="row g-0 align-items-center">
      <div class="col-md-3">
        <div class="px-3 py-3 py-md-0">
          ...
          >
            <div
              class="overflow-hidden position-relative rounded-5"
        ...
        <div class="card-body py-md-0 pe-md-5">
          ...
          <div class="row align-items-center justify-content-between mt-2">
            <ul
              ...
              v-if="
                doctor.doctor_categories && doctor.doctor_categories.length > 0
              "
            >
              <li
                class="me-2 lh-1 d-inline-block fs-3 text-dark fw-normal pe-2"
                v-for="(cat, i) in doctor.doctor_categories"
                :key="cat.id"
                v-bind:class="{
                  'border-end': i != doctor.doctor_categories.length - 1,
                }"
              >
                {{ cat.name }}
              </li>
          ...
          <div class="row align-items-center">
            ...
            >
              ...
              <span
                style="color: #2d2d2d"
                class="fs-4"
                v-if="doctor.experience == 1"
                >{{ doctor.experience }} {{ __("year") }}</span
              >
              <span style="color: #2d2d2d" class="fs-4" v-else
                >{{ doctor.experience }} {{ __("years") }}</span
              >
            </div>
    ...
    <!-- <div class="row mx-2 mb-3" v-if="doctor.appointment_types">
      <div
        class="col-md-4 pe-md-2 p-0 mb-2 mb-md-0"
        v-for="(schedule_type, index) in doctor.appointment_types"
        :key="index"
      >
        <div
          class="d-flex justify-content-center shadow-none btn btn-light px-2 py-3 align-items-center"
          role="button"
          @click="checkLoginAndRedirect(doctor, schedule_type.appointment_type)"
        >
          <i
            class="bi bi-camera-video-fill fs-5 me-2"
            v-if="schedule_type.type == 'video'"
          ></i>
          <i
            class="bi bi-mic-fill fs-5 me-2"
            v-if="schedule_type.type == 'audio'"
          ></i>
          <i
            class="bi bi-chat-dots-fill fs-5 me-2"
