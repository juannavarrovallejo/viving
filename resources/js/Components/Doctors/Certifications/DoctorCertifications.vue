<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="doctor_certifications.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __("no record found") }}
          </td>
        </tr>
        <tr
          v-for="certification in doctor_certifications.data"
          :key="certification.id"
        >
          <td class="text-center text-paragraphcolor fs-4 fw-normal">
            {{ certification.name }}
          </td>
      ...
      <template #Pagination>
        <TablePagination
          ...
          :meta="doctor_certifications.meta"
        ></TablePagination>
      </template>
    </Table>
  </div>
...
import AddEditCertificationModal from "@/Components/Doctors/Certifications/AddEditCertificationModal.vue";
import ViewCertificationModal from "@/Components/Doctors/Certifications/ViewCertificationModal.vue";
import DeleteCertificationModal from "@/Components/Doctors/Certifications/DeleteCertificationModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getDoctorCertifications(loading_more);
    },
    clearModalData() {
      this.modal_certification = null;
      this.key++;
    },
    getDoctorCertifications(loading_more) {
      axios
        .post(this.route("doctors.doctor_certifications.filter"), this.filter)
        .then((res) => {
          const data = res.data.data;
          if (loading_more) {
            this.doctor_certifications.data =
              this.doctor_certifications.data.concat(data.data);
          } else {
            this.doctor_certifications.data = data.data;
          }
          this.doctor_certifications.links = data.links;
          this.doctor_certifications.meta = data.meta;
          this.fetching = false;
        });
    },
  },
