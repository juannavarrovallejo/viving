    ...
    <Table v-if="!this.fetching">
      ...
      <template #TableHeaderButtons>
        ...
        >
          ...
          <button
            type="button"
            id="addEditEventModalButton"
            class="border-0 fs-3 fw-bold bg-transparent text-primary"
            data-bs-toggle="modal"
            @click="modal_event = null"
            data-bs-target="#addEditEventModal"
          >
            <img src="@/images/icons/plus.svg" width="25" class="mb-1 me-2" alt="" />
            {{ __("add") }} {{ __("events") }}
          </button>
      ...
      <template #TableBody>
        <tr v-if="doctor_events.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __("no record found") }}
          </td>
        </tr>
        <tr v-for="event in doctor_events.data" :key="event.id">
          ...
          <td class="align-middle">
            <span v-if="event.is_active" class="text-success fs-4 fw-bold">{{
              __("active")
            }}</span>
          ...
          <td class="align-middle">
            <div class="d-flex justify-content-center">
              <button
                type="button"
                class="bg-transparent text-primary fs-3 border-0 lh-1 p-1 me-2"
                data-bs-toggle="modal"
                @click="modal_event = event"
              ...
              <button
                type="button"
                class="bg-transparent border-0 fs-3 lh-1 p-2 me-2"
                style="color: #e2ae28"
                data-bs-toggle="modal"
                @click="modal_event = event"
                data-bs-target="#addEditEventModal"
              ...
              <button
                type="button"
                class="bg-transparent border-0 fs-3 px-1 lh-1 py-1"
                style="color: #fa6b6b"
                data-bs-toggle="modal"
                @click="modal_event = event"
...
import AddEditEventModal from "@/Components/Doctors/Events/AddEditEventModal.vue";
import ViewEventModal from "@/Components/Doctors/Events/ViewEventModal.vue";
import DeleteEventModal from "@/Components/Doctors/Events/DeleteEventModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getDoctorEvents(loading_more);
    },
    clearModalData() {
      this.modal_event = null;
      this.key++;
    },
    getDoctorEvents(loading_more) {
      axios.post(this.route("doctors.doctor_events.filter"), this.filter).then((res) => {
        const data = res.data.data;

        if (loading_more) {
          this.doctor_events.data = this.doctor_events.data.concat(data.data);
        } else {
          this.doctor_events.data = data.data;
        }
        this.doctor_events.links = data.links;
        this.doctor_events.meta = data.meta;
        this.fetching = false;
      });
    },
  },
