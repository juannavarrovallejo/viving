<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="doctor_broadcasts.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __("no record found") }}
          </td>
        </tr>
        <tr v-for="broadcast in doctor_broadcasts.data" :key="broadcast.id">
          ...
          <td class="align-middle">
            <span v-if="broadcast.is_active" class="text-success fs-4 fw-bold">{{
              __("active")
            }}</span>
...
import AddEditBroadcastModal from "@/Components/Doctors/Broadcasts/AddEditBroadcastModal.vue";
import ViewBroadcastModal from "@/Components/Doctors/Broadcasts/ViewBroadcastModal.vue";
import DeleteBroadcastModal from "@/Components/Doctors/Broadcasts/DeleteBroadcastModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getDoctorBroadcasts(loading_more);
    },
    clearModalData() {
      this.modal_broadcast = null;
      this.key++;
    },
    getDoctorBroadcasts(loading_more) {
      axios
        .post(this.route("doctors.doctor_broadcasts.filter"), this.filter)
        .then((res) => {
          const data = res.data.data;
          if (loading_more) {
            this.doctor_broadcasts.data = this.doctor_broadcasts.data.concat(data.data);
          } else {
            this.doctor_broadcasts.data = data.data;
          }
          this.doctor_broadcasts.links = data.links;
          this.doctor_broadcasts.meta = data.meta;
          this.fetching = false;
        });
    },
  },
