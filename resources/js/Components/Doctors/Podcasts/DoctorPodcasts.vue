<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="doctor_podcasts.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __("no record found") }}
          </td>
        </tr>
        <tr v-for="podcast in doctor_podcasts.data" :key="podcast.id">
...
import AddEditPodcastModal from "@/Components/Doctors/Podcasts/AddEditPodcastModal.vue";
import ViewPodcastModal from "@/Components/Doctors/Podcasts/ViewPodcastModal.vue";
import DeletePodcastModal from "@/Components/Doctors/Podcasts/DeletePodcastModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getDoctorPodcasts(loading_more);
    },
    clearModalData() {
      this.modal_podcast = null;
      this.key++;
    },
    getDoctorPodcasts(loading_more) {
      axios
        .post(this.route("doctors.doctor_podcasts.filter"), this.filter)
        .then((res) => {
          const data = res.data.data;
          if (loading_more) {
            this.doctor_podcasts.data = this.doctor_podcasts.data.concat(
              data.data
            );
          } else {
            this.doctor_podcasts.data = data.data;
          }
          this.doctor_podcasts.links = data.links;
          this.doctor_podcasts.meta = data.meta;
          this.fetching = false;
        });
    },
  },
