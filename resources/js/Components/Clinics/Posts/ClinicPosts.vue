<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="clinic_posts.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __("no record found") }}
          </td>
        </tr>
        <tr v-for="post in clinic_posts.data" :key="post.id">
...
import AddEditPostModal from "@/Components/Clinics/Posts/AddEditPostModal.vue";
import ViewPostModal from "@/Components/Clinics/Posts/ViewPostModal.vue";
import DeletePostModal from "@/Components/Clinics/Posts/DeletePostModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getClinicPosts(loading_more);
    },
    clearModalData() {
      this.modal_post = null;
      this.key++;
    },
    getClinicPosts(loading_more) {
      axios
        .post(this.route("clinics.clinic_posts.filter"), this.filter)
        .then((res) => {
          const data = res.data.data;
          if (loading_more) {
            this.clinic_posts.data = this.clinic_posts.data.concat(data.data);
          } else {
            this.clinic_posts.data = data.data;
          }
          this.clinic_posts.links = data.links;
          this.clinic_posts.meta = data.meta;
          this.fetching = false;
        });
    },
  },
