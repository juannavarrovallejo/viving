<template>
    ...
    <Modal maxWidth="lg" :id="id">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ modalData ? __('update'):__('create') }} {{__('doctor')}}</h5>
...
<!--
                ...
                <div class="form-group mb-3">
                    ...
                    <input id="clinic-image" ref="formImage"  @change="onFileChange" type="file" />
                </div>
                <div>
                    <!-- Cropped image previewer -->
                        <ImageCropperModal :show="showImportModal" id="doctorImageCropModal" :image_url="image_url" @cropImage="cropImage" ></ImageCropperModal>
...
export default defineComponent({
    ...
    async created(){
        await this.getDoctorCategories();
    ...
    methods:{
        ...
        create(){
            this.form.post(this.route('clinics.clinic_doctors.store'), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
        ...
        update(){
            this.form.transform((data) => ({
                    ...data,
                    _method: "PUT",
                })).post(this.route('clinics.clinic_doctors.update',{clinic_doctor:this.modalData.id}), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
         ...
         async getDoctorCategories() {
          await axios.get(this.route("getApiDoctorMainCategories")).then((res) => {
            this.doctor_categories = res.data.data;
            if (this.doctor_categories.length > 0) {
                this.form.doctor_categories.push(
                    this.doctor_categories[0].id
                )
            }
    ...
    modalData: {
      handler() {
        ...
        if(!this.modalData){
            ...
            this.form.doctor_categories = this.modalData.doctor_category_ids
