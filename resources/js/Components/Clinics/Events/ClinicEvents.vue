<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="clinic_events.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __("no record found") }}
          </td>
        </tr>
        <tr v-for="event in clinic_events.data" :key="event.id">
...
import AddEditEventModal from "@/Components/Clinics/Events/AddEditEventModal.vue";
import ViewEventModal from "@/Components/Clinics/Events/ViewEventModal.vue";
import DeleteEventModal from "@/Components/Clinics/Events/DeleteEventModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getClinicEvents(loading_more);
    },
    clearModalData() {
      this.modal_event = null;
      this.key++;
    },
    getClinicEvents(loading_more) {
      axios
        .post(this.route("clinics.clinic_events.filter"), this.filter)
        .then((res) => {
          const data = res.data.data;
          if (loading_more) {
            this.clinic_events.data = this.clinic_events.data.concat(data.data);
          } else {
            this.clinic_events.data = data.data;
          }
          this.clinic_events.links = data.links;
          this.clinic_events.meta = data.meta;
          this.fetching = false;
        });
    },
  },
