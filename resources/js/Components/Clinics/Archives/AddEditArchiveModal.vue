    ...
    <Modal maxWidth="lg" :id="id">
      <div class="modal-content rounded-50">
        <div class="modal-header py-4 px-5 border-0">
          ...
          >
            {{ modalData ? __("update") : __("create") }} {{ __("archive") }}
          </h5>
          <button
            :id="id + 'close'"
            type="button"
          ...
          <div class="row">
            ...
            >
              ...
              >
                <button
                  ...
                  :id="`archive-nav-${lang.code}-tab`"
                  data-bs-toggle="tab"
                  :data-bs-target="`#archive-nav-${lang.code}`"
                  type="button"
                  role="tab"
                  :aria-controls="`archive-nav-${lang.code}`"
                  aria-selected="true"
                >
                  {{ lang.name }}
                </button>
            ...
            >
              <div
                ...
                :id="`archive-nav-${lang.code}`"
                role="tabpanel"
                :aria-labelledby="`archive-nav-${lang.code}-tab`"
              >
                <div class="col-md-12">
            ...
            <div class="col-md-6">
              <div class="form-group mb-3">
                ...
                >
                <select
                  v-model="form.archive_category_id"
                  class="form-select"
                  aria-label="category"
                >
                  <option value="">{{ __("select") }}</option>
                  <option
                    v-for="cat in this.archive_categories"
                    :key="cat.id"
                    :value="cat.id"
                  >
                    {{ cat.name }}
...
export default defineComponent({
  ...
  methods: {
    ...
    computed: {},
    create() {
      this.form.post(this.route("clinics.clinic_archives.store"), {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    update() {
      this.form
        ...
        .post(
          this.route("clinics.clinic_archives.update", {
            clinic_archive: this.modalData.id,
          }),
          {
            preserveState: true,
            preserveScroll: true,
