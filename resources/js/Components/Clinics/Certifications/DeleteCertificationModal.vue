<template>
    ...
    <Modal maxWidth="lg" :id="id">
            ...
            <div class="modal-header border-0 px-5 mt-3">
                <h5 class="modal-title text-primary fs-1 fw-bold" id="exampleModalLabel">{{ __('delete') }} {{ __('certification') }}</h5>
...
export default {
    ...
    methods:{
        remove(){
            this.form.delete(this.route('clinics.clinic_certifications.destroy',this.modalData.id), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
